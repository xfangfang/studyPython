        ...
        for sock in r:
            if sock == server:
                ...
                if WebSocket.handshake(conn):
                    ...
                    sendall("当前在线人数:"+str(len(socket_list)-1))
            else:
                try:
                    data = WebSocket.recv(sock)
                    if not data:
                        print sock.getpeername()
                        socket_name.pop(sock.getpeername())
                        ...
                        d = str(data).split(":")
                        if len(d)==2:
                            socket_name[sock.getpeername()]=d[0]
                            if d[1]=="1":
                                f = ""
                                for i in socket_name:
                                    f+=socket_name[i]+"</br>"
                                sendall("当前在线人数:"+str(len(socket_list)-1)+"</br>"+f)
                except socket.error:
